...
class CommanderTCPHandler(SocketServer.BaseRequestHandler):
    ...
    def send_servoc(self, waypoint_id, pose, a, v, r):
        assert(len(pose) == 6)

        params = [MSG_SERVOC, waypoint_id] + \
                 [MULT_cartesian * p for p in pose] + \
                 [MULT_cartesian * a] + \
                 [MULT_cartesian * v] + \
                 [MULT_cartesian * r]
        buf = struct.pack("!%ii" % len(params), *params)
...
class URCartesianTrajectoryFollower(object):
    RATE = 0.02
...
def main():
    ...
    if len(args) < 1:
        ...
        # joint servoing is the default action server
        action_server_type = "joint"
    elif len(args) == 2:
        robot_hostname = args[0]
        reverse_port = DEFAULT_REVERSE_PORT
        global prefix
        ...
        while not rospy.is_shutdown():
            ...
            if getConnectedRobot(wait=False):
                ...
                if action_server:
                    ...
                    if action_server_type == "joint":
                        action_server = URTrajectoryFollower(
                            r, rospy.Duration(1.0))
                    else:
                        action_server = URCartesianTrajectoryFollower(
                            r, rospy.Duration(1.0))
                    action_server.start()
